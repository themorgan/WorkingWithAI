#!/usr/bin/env python3
# Last updated: 2026-08-28 22:40:00 (Buenos Aires) by Morgan F, to version 3
...
process/personal/README.md#automation-issues: any unattended job in this repo that hits
...
permission -- set both in the calling workflow job, e.g.:
  ...
  steps:
    - env:
      ...
      run: python3 process/personal/tools/report_automation_issue.py ...

Fails gracefully (process/personal/README.md#fail-gracefully): if `gh` isn't
authenticated, isn't installed, or the API call fails for any reason, this
prints a clear WARN to stderr and exits 0 rather than failing the calling
job -- a failure to *report* a blocker should never itself become a second,
more confusing blocker (the run's own `::warning::` annotation, left in
place by the caller, is the fallback that still gets through).
