<!-- Last updated: 2026-09-04 10:49:37 (Buenos Aires) by Morgan F, to version 53 -->
...
- [x] **Move the main content documents into `docs/`, add uniform
  ...
  into [docs-team/doc-recipes/HUMANS_AT_OUR_BEST.recipe.md](content/doc-recipes/HUMANS_AT_OUR_BEST.recipe.md) — a standing rule
...
- [x] **Rename `docs/` to `docs-team/`.** Decided 2026-09-02 → done, same
  ...
  documents and [docs-team/doc-recipes/HUMANS_AT_OUR_BEST.recipe.md](content/doc-recipes/HUMANS_AT_OUR_BEST.recipe.md)
  ...
  (3).
- [x] **Rename `docs-team/` to `content/`.** Decided 2026-09-04 → done,
  same day. Morgan's reasoning: `docs-team/` sat too close, by name, to
  `docs-team/doc-recipes/` — the insider, technical per-document
  rule-recipe subdirectory living right inside it — inviting the two to
  be mixed up; `content` names what the directory holds (this repo's
  main content documents) without echoing `doc-recipes` at all. All eight
  content documents and their nested `doc-recipes/` subdirectory moved
  via `git mv`; every repo-wide link updated to match ([AGENTS.md](AGENTS.md),
  [MAP.md](MAP.md), [GLOSSARY.md](GLOSSARY.md), [README.md](README.md),
  [GETTING_STARTED.md](GETTING_STARTED.md), [process/PRECEDENT_MIGRATION.md](process/PRECEDENT_MIGRATION.md),
  and this file's current-state links). Same precedent as the two rename
  entries above: historical prose describing the directory by a past
  name (`docs/`, `docs-team/`) was left as written — a record of what
  each move actually did — while every link, in every entry including
  those two, was repointed to keep resolving.
  `tools/light_check.py`'s own incident comment (describing a session
  that once reorganized `docs-team/HUMANS_AT_OUR_BEST.md`) is prose, not
  a link, so it kept the old name too, same reasoning.
